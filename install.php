...
$options->generator = 'Dabao_Blog ' . Typecho_Common::VERSION;
        ...
        <div class="col-mb-12 col-tb-8 col-tb-offset-2">
            ...
            <?php if (isset($_GET['finish'])) : ?>
                ...
                <div class="typecho-install-body">
                    ...
                    <p><?php _e('希望您能尽情享用 blog 带来的乐趣!'); ?></p>
                </div>
                <?php endif;?>
            ...
            <?php
                                    ...
                                    try {
                                        ...
                                        $installDb->query($installDb->insert('table.options')->rows(array('name' => 'keywords', 'user' => 0, 'value' => 'dabao,php,blog')));
                                        ...
                                        $installDb->query($installDb->insert('table.options')->rows(array('name' => 'allowRegister', 'user' => 0, 'value' => 1)));
                                        ...
                                        /** 初始内容 */
                                        $installDb->query($installDb->insert('table.contents')->rows(array('title' => _t('欢迎使用 Blog'), 'slug' => 'start', 'created' => Typecho_Date::gmtTime(), 'modified' => Typecho_Date::gmtTime(),
                                        ...
                                        'text' => '<!--markdown-->' . _t('本页面由 blog 创建, 这只是个测试页面.'), 'authorId' => 1, 'order' => 0, 'type' => 'page', 'status' => 'publish', 'commentsNum' => 0, 'allowComment' => 1,
                                        'allowPing' => 1, 'allowFeed' => 1, 'parent' => 0)));

                                        /** 初始评论 */
                                        $installDb->query($installDb->insert('table.comments')->rows(array('cid' => 1, 'created' => Typecho_Date::gmtTime(), 'author' => 'Dabao', 'ownerId' => 1, 'url' => 'http://localhost/',
                                        'ip' => '127.0.0.1', 'agent' => $options->generator, 'text' => '欢迎加入 dabao blog', 'type' => 'comment', 'status' => 'approved', 'parent' => 0)));
                                        ...
                                        'url' => 'http://localhost', 'screenName' => $config['userName'], 'group' => 'administrator', 'created' => Typecho_Date::gmtTime())));
...
<p><button name="created" value="1" type="submit" class="btn primary">创建完毕, 继续安装 &raquo;</button></p></div>
                        ...
                        <ul class="typecho-option">
                            ...
                            <input type="text" class="text" name="dbPrefix" id="dbPrefix" value="<?php _v('dbPrefix', 'blog_'); ?>" />
                            <p class="description"><?php _e('默认前缀是 "blog_"'); ?></p>
                            </li>
                        </ul>
            ...
            <?php  else: ?>
                ...
                <h1 class="typecho-install-title"><?php _e('欢迎使用 blog'); ?></h1>
                <div class="typecho-install-body">
                <h2><?php _e('安装说明'); ?></h2>
                ...
                <h2><?php _e('许可及协议'); ?></h2>
                <p><?php _e('Blog 基于 <a href="http://www.gnu.org/copyleft/gpl.html">GPL</a> 协议发布, 我们允许用户在 GPL 协议许可的范围内使用, 拷贝, 修改和分发此程序.'); ?>
                <?php _e('在GPL许可的范围内，您可以自由地将其用于商业以及非商业用途.'); ?></p>
                <p><?php _e('Blog 软件由其社区提供支持, 核心开发团队负责维护程序日常开发工作以及新特性的制定.'); ?>
